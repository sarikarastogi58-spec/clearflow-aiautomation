...
test("server-renders the ClearFlow dashboard", async () => {
  const response = await render();
  assert.equal(response.status, 200);
  assert.match(response.headers.get("content-type") ?? "", /^text\/html\b/i);
  const html = await response.text();
  assert.match(html, /<title>ClearFlow — AI Automation<\/title>/i);
  assert.match(html, /ClearFlow/);
  assert.match(html, /AI Automation Control Centre/i);
  assert.doesNotMatch(html, /codex-preview|Your site is taking shape/i);
});

test("outreach and webhook routes enforce consent and opt-out", async () => {
