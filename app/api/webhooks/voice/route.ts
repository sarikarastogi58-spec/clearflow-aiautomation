...
export async function POST(request: Request) {
  ...
  if (speech) {
    await recordInbound("voice", from, speech, `${callSid}:${String(form.get("SequenceNumber") ?? Date.now())}`).catch(() => undefined);
    try { reply = await generateSupportReply(speech, `Voice caller: ${from}. Keep the spoken reply under 40 words.`); }
